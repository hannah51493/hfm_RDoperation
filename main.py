...
# 탭 구성
# =====================
tab1, tab2, tab3, tab4 = st.tabs(["🔍 STEP 1 : 광고코드 추출", "📊 STEP 2 : RD 생성", "📖 가공 규칙 확인", "⚙️ 파라미터 규칙 상세"])
...
with tab1:
    ...
    # Code_prefix 시트에서 브랜드/매체 코드 읽기
    prefix_df = config_sheets.get('Code_prefix', pd.DataFrame())
    if not prefix_df.empty and '구분' in prefix_df.columns and '코드' in prefix_df.columns:
        brand_codes = list(prefix_df[prefix_df['구분'].str.strip() == '브랜드']['코드'].str.strip())
        media_codes = list(prefix_df[prefix_df['구분'].str.strip() == '매체']['코드'].str.strip())
        if brand_codes and media_codes:
            b = '|'.join(brand_codes)
            m = '|'.join(media_codes)
            code_pattern = rf'(?:{b})(?:{m})\d{{4}}|(?:{m})(?:{b})\d{{4}}'
        else:
            code_pattern = r'(?:HF|TM|TJ)[A-Z]{2}\d{4}'
    else:
        code_pattern = r'(?:HF|TM|TJ)[A-Z]{2}\d{4}'

    def extract_codes(index_val):
        full_matches = re.findall(code_pattern, str(index_val))
...
with tab2:
    ...
    if st.button("▶ RD 생성 실행", key='step2'):
        ...
        # 매체명은 정합성 검증에 필요하므로 col_filter 에서 제외
        safe_filter  = [c for c in col_filter if c != '매체명']
        ordered_cols = [c for c in STANDARD_COL_ORDER if c in final_df.columns and c not in safe_filter]
        extra_cols   = [c for c in final_df.columns if c not in STANDARD_COL_ORDER and c not in safe_filter]
...
# =====================
with tab3:
    st.header("STEP 3 : 가공 규칙 확인")
    st.caption("매체별 인덱스 생성 규칙 및 표준칼럼 가공 방식을 확인합니다.")

    # custom 함수 설명
    CUSTOM_DESC = {
        'ga4_index'       : """소스매체 기준 조건 분기
• Apple / search, ig / paid → 소스매체 + 캠페인
• google / display + shopping 캠페인 → 소스매체 + 캠페인 + 검색어
• 나머지 → 소스매체 + 캠페인 + 컨텐츠
• (not set) 은 문자 그대로 유지, nan/빈값만 제거""",
        'af_event_count'  : "AF 원본에서 Event Name 기준으로 해당 이벤트 발생 시 1, 아니면 0 카운트",
        'af_revenue'      : "AF 원본에서 af_purchase 이벤트의 Event Revenue 값 합산, 나머지는 0",
        'naver_bsa_daily' : "Config_BSAcost 시트에서 타겟팅(광고세트) 기준으로 해당 월 일예산 lookup",
        'meta_status'     : "광고 게재 열 값이 'active' 이면 ON, 나머지는 OFF",
        'meta_purchase'   : """광고상품 기준 조건 분기
• 앱리마케팅 / Meta APP → 구매 (AF)
• DPA → 구매(WEB) + 구매 (AF)
• 나머지 → 구매(WEB) + 구매(APP)""",
        'meta_revenue'    : """광고상품 기준 조건 분기
• 앱리마케팅 / Meta APP → 매출액 (AF)
• DPA → 매출액(WEB) + 매출액 (AF)
• 나머지 → 매출액(WEB) + 매출액(APP)""",
    }
    ...
    PROC_TYPE_DESC = {
        'map'          : '원본 칼럼명 그대로 매핑',
        'map_idx'      : '열 인덱스 위치로 매핑',
        'static'       : '고정값 입력',
        'date_format'  : '날짜 형식 변환',
        'date_extract' : '날짜 파생 열 생성 (week/month/year)',
        'concat'       : '열 병합 ( | 구분자로 조합)',
        'to_numeric'   : '숫자 형식 변환',
        'sum_cols'     : '지정 열 합산',
        'lookup'       : '단일 키로 참조 시트에서 값 매핑',
        'lookup_multi' : '복합 키(Date+광고코드)로 GA4/AF 데이터 매핑',
        'custom'       : 'custom_functions.py 에 정의된 함수 실행',
    }

    if config_col_df.empty:
        st.info("👈 좌측에서 설정 파일을 업로드해주세요.")
    else:
        config_col_df.columns = config_col_df.columns.str.strip()
        media_list = sorted(config_col_df['매체명'].str.strip().unique().tolist())

        selected = st.selectbox("매체 선택", options=media_list)

        if selected:
            media_cfg = config_col_df[config_col_df['매체명'].str.strip() == selected].copy()

            # 인덱스 규칙 강조 표시
            index_cols = ['인덱스(매체)', '인덱스(ga4)', '인덱스(AF)']
            idx_data   = media_cfg[media_cfg['표준칼럼명'].str.strip().isin(index_cols)]

            if not idx_data.empty:
                st.subheader("🔑 인덱스 생성 규칙")
                for _, row in idx_data.iterrows():
                    std   = str(row['표준칼럼명']).strip()
                    ptype = str(row['가공유형']).strip()
                    src   = str(row['원본칼럼명']).strip() if pd.notna(row['원본칼럼명']) else '-'
                    param = str(row['파라미터']).strip()  if pd.notna(row['파라미터'])  else '-'

                    with st.expander(f"**{std}**", expanded=True):
                        st.markdown(f"**가공유형:** `{ptype}`")
                        if ptype == 'concat':
                            parts = param.split('|')
                            st.markdown(f"**조합 규칙:** {' + '.join([f'`{p}`' for p in parts])}")
                        elif ptype == 'map':
                            st.markdown(f"**원본 열:** `{src}`")
                        elif ptype == 'custom':
                            desc = CUSTOM_DESC.get(param, '설명 없음')
                            st.markdown(f"**함수:** `{param}`")
                            st.markdown(f"**규칙:**")
            ...
            for _, row in media_cfg.iterrows():
                ...
                if ptype == 'map':
                    ...
                    rule = ' + '.join(param.split('|'))
                elif ptype == 'date_format':
                    rule = f"날짜변환 ({src} → {param})"
                elif ptype == 'date_extract':
                    rule = f"{param} 추출"
                elif ptype == 'lookup':
                    parts = param.split('|')
                    rule  = f"{parts[0]} 시트에서 {parts[1]} 기준 → {parts[2]}" if len(parts) == 3 else param
                elif ptype == 'lookup_multi':
                    parts = param.split('|')
                    rule  = f"{parts[0]} 시트에서 {parts[1]} 기준 → {parts[2]}" if len(parts) == 3 else param
                elif ptype == 'sum_cols':
                    rule = ' + '.join(param.split('|'))
                elif ptype == 'custom':
                    rule = CUSTOM_DESC.get(param, param)[:50] + '...' if len(CUSTOM_DESC.get(param, param)) > 50 else CUSTOM_DESC.get(param, param)
                else:
                    rule = param or src
...
# =====================
with tab4:
    st.header("⚙️ 파라미터 규칙 상세")
    st.caption("custom / date_extract 등 복잡한 가공 규칙의 상세 내용을 확인합니다.")
    ...
    PARAM_RULES = {
        'custom': {
            'ga4_index': {
                'title': 'GA4 인덱스 생성',
                'description': '소스/매체 값 기준으로 조건 분기하여 인덱스 생성',
                'rules': [
                    ('소스매체 = Apple / search 또는 ig / paid', '소스매체 + 캠페인'),
                    ('소스매체 = google / display AND 캠페인에 shopping 포함', '소스매체 + 캠페인 + 검색어'),
                    ('나머지 전부', '소스매체 + 캠페인 + 컨텐츠'),
                ],
                'notes': '(not set) 은 문자 그대로 유지 / nan, 빈값만 제거',
                'file': 'custom_functions.py → ga4_index()',
            },
            'af_event_count': {
                'title': 'AF 이벤트 카운트',
                'description': 'AF 원본의 Event Name 기준으로 해당 이벤트 발생 여부 카운트',
                'rules': [
                    ('(AF)install', 'Event Name = install → 1, 나머지 → 0'),
                    ('(AF)purchase', 'Event Name = af_purchase → 1, 나머지 → 0'),
                    ('(AF)re-attribution', 'Event Name = re-attribution → 1, 나머지 → 0'),
                    ('(AF)re-engagement', 'Event Name = re-engagement → 1, 나머지 → 0'),
                ],
                'notes': '표준칼럼명이 파라미터로 전달되어 해당 이벤트 자동 매핑',
                'file': 'custom_functions.py → af_event_count()',
            },
            'af_revenue': {
                'title': 'AF 매출 합산',
                'description': 'AF 원본에서 af_purchase 이벤트의 Event Revenue 합산',
                'rules': [
                    ('Event Name = af_purchase', 'Event Revenue 값 사용'),
                    ('나머지', '0'),
                ],
                'notes': '',
                'file': 'custom_functions.py → af_revenue()',
            },
            'naver_bsa_daily': {
                'title': 'Naver BSA 일예산 매핑',
                'description': 'Config_BSAcost 시트에서 타겟팅(광고세트) 기준으로 일예산 lookup',
                'rules': [
                    ('기준 시트', 'Config_BSAcost'),
                    ('매칭 키', '타겟팅 (광고세트명)'),
                    ('값', 'YYYYMM 열의 일예산'),
                ],
                'notes': '월별로 열이 우측으로 확장 / 숫자 변환 시 콤마 자동 제거',
                'file': 'custom_functions.py → naver_bsa_daily()',
            },
            'meta_status': {
                'title': 'Meta 상태 변환',
                'description': '원본 광고 게재 열 값을 ON/OFF로 변환',
                'rules': [
                    ('광고 게재 = active (대소문자 무관)', 'ON'),
                    ('나머지', 'OFF'),
                ],
                'notes': '',
                'file': 'custom_functions.py → meta_status()',
            },
            'meta_purchase': {
                'title': 'Meta 구매 산정',
                'description': '광고상품 기준으로 구매 데이터 출처 분기',
                'rules': [
                    ('앱리마케팅 / Meta APP', '구매 (AF)'),
                    ('DPA', '구매(WEB) + 구매 (AF)'),
                    ('나머지', '구매(WEB) + 구매(APP)'),
                ...
                'notes': '',
                'file': 'custom_functions.py → meta_purchase()',
            },
            'meta_revenue': {
                'title': 'Meta 매출액 산정',
                'description': '광고상품 기준으로 매출액 데이터 출처 분기',
                'rules': [
                    ('앱리마케팅 / Meta APP', '매출액 (AF)'),
                    ('DPA', '매출액(WEB) + 매출액 (AF)'),
                    ('나머지', '매출액(WEB) + 매출액(APP)'),
                ...
                'notes': '',
                'file': 'custom_functions.py → meta_revenue()',
            },
        },
        'date_extract': {
            'week': {
                'title': 'Week 주차 추출',
                'description': '해당 날짜가 속한 주의 월요일 날짜 기준으로 주차 표기',
                'rules': [
                    ('4/6 (일요일)', '→ 월요일 3/31 → 03/31주차'),
                    ('4/7 (월요일)', '→ 월요일 4/7 → 04/07주차'),
                    ('4/13 (일요일)', '→ 월요일 4/7 → 04/07주차'),
                ],
                'notes': 'MM/DD주차 형식 / 월요일 시작 기준',
                'file': 'main.py → apply_config_column() date_extract',
            },
            'month': {
                'title': 'Month 월 추출',
                'description': 'Date 열에서 월 추출',
                'rules': [('Date 열 기준', 'MM월 형식 (예: 03월)')],
                'notes': '',
                'file': 'main.py → apply_config_column() date_extract',
            },
            'year': {
                'title': 'Year 연도 추출',
                'description': 'Date 열에서 연도 추출',
                'rules': [('Date 열 기준', 'YYYY년 형식 (예: 2026년)')],
                'notes': '',
                'file': 'main.py → apply_config_column() date_extract',
            },
        },
    ...
    col_type, col_param = st.columns([1, 2])

    with col_type:
        proc_type = st.selectbox("가공유형 선택", options=list(PARAM_RULES.keys()))

    with col_param:
        param_options = list(PARAM_RULES[proc_type].keys())
        param = st.selectbox("파라미터 선택", options=param_options)

    if proc_type and param:
        rule = PARAM_RULES[proc_type][param]

        st.divider()
        st.subheader(f"📌 {rule['title']}")
        st.markdown(f"**설명:** {rule['description']}")
        st.markdown(f"**파일:** `{rule['file']}`")

        st.markdown("**조건 규칙:**")
        rule_df = pd.DataFrame(rule['rules'], columns=['조건', '결과값'])
        st.dataframe(rule_df, use_container_width=True, hide_index=True)

        if rule.get('notes'):
            st.info(f"📎 {rule['notes']}")
