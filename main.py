...
def apply_config_column(df, config_col_df, media_name, config_sheets, year_month=None):
    ...
    """
    media_config = config_col_df[config_col_df['매체명'] == media_name]
    result_df    = pd.DataFrame()

    for _, row in media_config.iterrows():
        std_col    = row['표준칼럼']
        proc_type  = row['가공유형']
        src_col    = row['원본칼럼명'] if pd.notna(row['원본칼럼명']) else ''
        param      = row['파라미터']  if pd.notna(row['파라미터'])  else ''
...
# 열 이름 공백 제거 (앞뒤 공백으로 인한 KeyError 방지)
config_file_df.columns = config_file_df.columns.str.strip()
...
for f in media_files:
    for _, cfg_row in config_file_df.iterrows():
        pattern = str(cfg_row['파일명패턴']).strip().replace('*', '.*')
...
for _, cfg_row in config_file_df.iterrows():
    ...
    if media in media_file_map:
        ...
        st.sidebar.error(f"🔴 {media} (파일 없음)")
